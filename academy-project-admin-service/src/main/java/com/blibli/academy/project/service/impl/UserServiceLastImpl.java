...
public class UserServiceLastImpl implements UserServiceLast {
    ...
    public List<Role> findRoleList() {
        return userDaoLast.findRoleList();
    }
     ...
     */
    @Override
    public Boolean deleteRoleModuleId(Long roleId) {
        return userDaoLast.deleteRoleModuleId(roleId);
    }
...
//    public Long getInsertRoleId(String roleName){return userDaoLast.getInsertRoleId(roleName);}
     ...
     */
    @Override
    public ResponseVO deleteModuleId(Long id) {
        if (userDaoLast.getRootModuleCount(id) != 0) {
            log.info("输出-----------"+userDaoLast.getRootModuleCount(id));
            return ResultUtil.error("有其他管理员正在使用此权限，不得删除");
        } else {
            userDaoLast.deleteModuleId(id);
            return ResultUtil.success("模块儿删除成功");
        }
    }
}
