...
public class AdminController {
    ...
    public ResponseVO submitLogin(@RequestBody UserService userService) {
        try {
            UsernamePasswordToken token = new UsernamePasswordToken(userService.getUsername(), userService.getPassword(),true);
    ...
    public ResponseVO userLogout() {
        ...
        currentUser.logout();
     return ResultUtil.success("清除redis并---退出成功");
    }
...
//    @RequiresPermissions(".account")
    ...
    public ResponseRowsVO index(){
        log.info("进入到这里了吗-------");
            ...
            log.info("获取用户的登陆名字---------"+loginName.loginName());
            log.info("获取用户 ID ---" + id);
